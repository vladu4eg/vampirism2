...
function lord_of_lightning_chain_lightning:OnSpellStart()
	...
	Timers:CreateTimer(DoUniqueString("ChainLightning"), {
		...
		callback = function()
			...
			units = FindUnitsInRadius(caster:GetTeamNumber(), target:GetOrigin(), target, bounce_range, DOTA_UNIT_TARGET_TEAM_ENEMY, 
			...
			dummy = CreateUnitByName("dummy_unit", targetVec, false, caster, caster, caster:GetTeam())
